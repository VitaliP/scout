...
describe("Integration test",()=>{
    ...
    const btn = findByAttr(wrapper,'btn-fetch');

    it('Render fetch btn by default', () => {
        expect(btn.text()).toEqual('Fetch data');
    });

    it('Render loading when btn clicked', () => {
        btn.simulate('click');
        wrapper.update();
        const loading = findByAttr(wrapper,'loading');
        expect(loading.text()).toBe('Loading...');
    });


    it('Render data table when fetch done', () => {
        moxios.wait(() => {
        ...
        return store.dispatch(fetchData())
            .then(() => {
                expect(wrapper.html()).toMatch(/<td>AUD<\/td>/);
            });
    });

    it('Render modal err when api error', () => {
        moxios.wait(() => {
            const request = moxios.requests.mostRecent();
            request.respondWith({
                status: 400,
        ...
        return store.dispatch(fetchData())
            .then(() => {
                wrapper.update();
                expect(wrapper.html()).toMatch(/Error fetching data | Error code = 400/);
            });
    });
